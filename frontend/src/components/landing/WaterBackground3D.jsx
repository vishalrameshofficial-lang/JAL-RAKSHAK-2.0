...
export default function WaterBackground3D() {
  ...
  useEffect(() => {
    ...
    const waterMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          // Compound sine/cosine displacement as specified:
          // z = sin(x * 0.4 + time) * 1.2 + cos(y * 0.3 + time * 0.8) * 0.8
          float x = worldPos.x;
          float y = worldPos.z; // planar horizontal axis after rotation

          float wave = sin(x * 0.4 + uTime) * 1.2 + cos(y * 0.3 + uTime * 0.8) * 0.8;
          
          // High-frequency capillary ripples for hydraulic realism
          wave += sin(x * 0.85 + y * 0.7 + uTime * 1.5) * 0.3;
          wave += cos(x * 1.3 - y * 1.1 + uTime * 2.1) * 0.15;

          // Localized mouse disturbance
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Specified opacity: 0.75 on peaks, subtle transparency in troughs
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      velocities[i * 3 + 1] = 0.012 + Math.random() * 0.024; // upward along Y
      velocities[i * 3 + 2] = (Math.random() - 0.5) * 0.015;

      // Half electric cyan (#00f0ff), half emerald (#10b981)
    ...
    const particleMat = new THREE.ShaderMaterial({
      ...
      transparent: true,
      depthWrite: false,
      blending: THREE.AdditiveBlending, // Soft additive blending as requested
    });
    ...
    const animate = () => {
      ...
      // Gentle plane tilt driven by mouse position
      ...
      // Particle upward float + brownian drift
    ...
    /* ─── 7. Throttled Window Resize Listener ─── */
    let resizeTimer;
    const handleResize = () => {
